...
def pipeline() -> None:
    ...
    modelfile = f'{path}/data/models/cars_pipe{datetime.now().strftime("%Y%m%d%H%M")}.pkl'
    with open(modelfile, 'wb') as file:
        dill.dump(pickle, file)
    logging.info(f'Model is saved as {modelfile}')
